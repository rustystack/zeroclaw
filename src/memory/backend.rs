...
const HDF5_PROFILE: MemoryBackendProfile = MemoryBackendProfile {
    key: "hdf5",
    label: "HDF5 — single-file agent memory with adaptive vector search (EdgeHDF5)",
    auto_save_default: true,
    uses_sqlite_hygiene: false,
    sqlite_based: false,
    optional_dependency: true,
...
pub fn memory_backend_profile(backend: &str) -> MemoryBackendProfile {
    match classify_memory_backend(backend) {
        ...
        MemoryBackendKind::Hdf5 => HDF5_PROFILE,
...
mod tests {
    ...
    fn classify_known_backends() {
        ...
        assert_eq!(classify_memory_backend("hdf5"), MemoryBackendKind::Hdf5);
